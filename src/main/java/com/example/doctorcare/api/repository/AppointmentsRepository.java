...
public interface AppointmentsRepository extends CrudRepository<AppointmentsEntity, Long> {
    ...
    Page<AppointmentsEntity> findByCreateDateAfterAndHospital(Long id,LocalDateTime before,
                                                              Pageable pageable);
    @Query(value = "select a.* from appointments a join time_doctors td on a.time_doctors_id=td.id join `user` u on td.doctor_id= u.id where hospital_cilinic_id=?1 and date(?2) <= td.date",nativeQuery = true)
    Page<AppointmentsEntity> findByCreateDateBeforeAndHospital(Long id,LocalDateTime after,
                                                               Pageable pageable);
    ...
    Page<AppointmentsEntity> findByCreateDateBetweenAndHospital(Long id,LocalDateTime before, LocalDateTime after,Pageable pageable);
